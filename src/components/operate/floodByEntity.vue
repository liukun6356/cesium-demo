<template>
    <!-- 特效分析 -->
    <div class="folldByEntity-container">
        <button class="btn" @click="fog">雾效果</button>
        <button class="btn" @click="rain">下雨效果</button>
        <button class="btn" @click="snow">下雪效果</button>
        <button class="btn" @click="removeStage">关闭当前效果</button>
        <button class="btn" @click="snowCover">雪覆盖效果</button>
        <button class="btn" @click="offSnowCover">关闭雪覆盖效果</button>
        <button class="btn" @click="circleScan">扩散效果</button>
        <button class="btn" @click="radarScan">扫描效果</button>
    </div>
</template>
...
export default {
    ...
    methods: {
        fog() {
            this.removeStage();
            let fogEffect = new das3d.scene.FogEffect({
                viewer: window.dasViewer,
                maxHeight: 20000, //大于此高度后不显示
                fogByDistance: new Cesium.Cartesian4(100, 0.0, 9000, 0.9), //最大能见度5000米
                color: Cesium.Color.WHITE,
                show: true, //是否开启效果
            });
            this.lastStage = fogEffect.FogStage;
        },
        rain() {
            this.removeStage();
            let rain = new Cesium.PostProcessStage({
                fragmentShader: das3d.shader.rain,
            });
            window.dasViewer.scene.postProcessStages.add(rain);
            this.lastStage = rain;
        },
        snow() {
            this.removeStage();
            let snow = new Cesium.PostProcessStage({
                fragmentShader: das3d.shader.snow,
            });
            window.dasViewer.scene.postProcessStages.add(snow);
            this.lastStage = snow;
        },
        removeStage() {
            if (this.lastStage)
                window.dasViewer.scene.postProcessStages.remove(this.lastStage);
            this.lastStage = null;
        },
        snowCover() {
            let snow = new das3d.scene.SnowCover({
                viewer: window.dasViewer,
                alpha: 0.5,
            });
            this.snowCoverStage = snow;
        },
        offSnowCover() {
            this.snowCoverStage.show = false;
        },
        circleScan() {
            this.removeStage();
            var point = das3d.point.getCenter(window.dasViewer);
            var cartographicCenter = new Cesium.Cartographic(
                Cesium.Math.toRadians(point.x),
                Cesium.Math.toRadians(point.y),
                point.z
            );
            var scanColor = new Cesium.Color(1.0, 0.0, 0.0, 1);

            this.lastStage = addCircleScanPostStage(
                window.dasViewer,
                cartographicCenter,
                1500,
                scanColor,
        ...
        radarScan() {
            ...
            var scanColor = new Cesium.Color(1.0, 0.0, 0.0, 1);
            this.lastStage = addRadarScanPostStage(
                window.dasViewer,
                cartographicCenter,
                1000,
                scanColor,
                ...
                var ScanPostStage = new Cesium.PostProcessStage({
                    ...
                    uniforms: {
                        ...
                        u_scanLineNormalEC: function () {
                            ...
                            var tempTime =
                                ((new Date().getTime() - _time) % duration) /
                                duration;
