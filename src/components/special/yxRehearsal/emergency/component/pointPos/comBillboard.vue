...
export default {
  ...
  methods: {
    addDivpoint() {
      const divpoint = new das3d.DivPoint(window.dasViewer, {
        html: `<div class="divpointBox2">${this.data.type} </div>`,
        popup: {
          html: this.data.htmlStr?`<div class="pointPopupBox">${this.data.htmlStr}</div>`:'',
          anchor: [0, -25],
        },
        horizontalOrigin: Cesium.HorizontalOrigin.CENTER,
        verticalOrigin: Cesium.VerticalOrigin.BOTTOM,
        position: Cesium.Cartesian3.fromDegrees(this.data.location[0], this.data.location[1], 1700),
        distanceDisplayCondition: new Cesium.DistanceDisplayCondition(0, 100000), //按视距距离显示
        click: () => {
          this.$emit('typeSelect',this.data.type)
        },
      })
  ...
  mounted() {
    pointPosDatasource = new Cesium.CustomDataSource("pointPosEntity" + this.data.id);
