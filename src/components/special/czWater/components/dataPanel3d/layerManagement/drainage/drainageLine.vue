...
export default {
  mounted() {
    ...
    const WmsMapService = new Cesium.WebMapServiceImageryProvider({
      ...
      layers:'chenzhou:space_drainage_line', // 服务名称
      parameters: {
        service: 'WMS',
        format:  'image/png',
        srs: 'EPSG:4326',
  ...
  destroyed() {
    const viewer = window.dasViewer;
    const layer = viewer.imageryLayers._layers.find(layer=>layer._imageryProvider._layers ==='chenzhou:space_drainage_line')
