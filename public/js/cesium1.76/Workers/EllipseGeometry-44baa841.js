...
define(['exports', './when-ca071c93', './Check-802a3f11', './Math-439e03ae', './Cartesian2-be0369bc', './Transforms-01f84fc8', './ComponentDatatype-2a7010d1', './GeometryAttribute-76dbac20', './GeometryAttributes-3b5bdbdf', './GeometryPipeline-85192c0e', './IndexDatatype-f1ac8920', './GeometryOffsetAttribute-20609f84', './VertexFormat-07d7ff51', './EllipseGeometryLibrary-84ce7a28', './GeometryInstance-6159d9d1'], function (exports, when, Check, _Math, Cartesian2, Transforms, ComponentDatatype, GeometryAttribute, GeometryAttributes, GeometryPipeline, IndexDatatype, GeometryOffsetAttribute, VertexFormat, EllipseGeometryLibrary, GeometryInstance) { 'use strict';
  ...
  function computeTopBottomAttributes(positions, options, extrude) {
    ...
    var minTexCoord = Cartesian2.Cartesian2.fromElements(
      Number.POSITIVE_INFINITY,
      Number.POSITIVE_INFINITY,
      scratchMinTexCoord
    );
    var maxTexCoord = Cartesian2.Cartesian2.fromElements(
    ...
    for (var i = 0; i < length; i += 3) {
      ...
      if (vertexFormat.st) {
        var rotatedPoint = Transforms.Matrix3.multiplyByVector(
          textureMatrix,
          position,
          scratchCartesian2
        );
    ...
    if (vertexFormat.position) {
      var finalPositions = EllipseGeometryLibrary.EllipseGeometryLibrary.raisePositionsToHeight(
        positions,
        options,
        extrude
      );
  ...
  function computeEllipse(options) {
    var center = options.center;
    boundingSphereCenter = Cartesian2.Cartesian3.multiplyByScalar(
      options.ellipsoid.geodeticSurfaceNormal(center, boundingSphereCenter),
      options.height,
      boundingSphereCenter
    );
    boundingSphereCenter = Cartesian2.Cartesian3.add(
      center,
      boundingSphereCenter,
      boundingSphereCenter
    );
  ...
  function computeWallAttributes(positions, options) {
    ...
    for (var i = 0; i < length; i += 3) {
      ...
      extrudedPosition = Cartesian2.Cartesian3.add(
        extrudedPosition,
        scaledNormal,
        extrudedPosition
      );
      ...
      if (vertexFormat.normal || vertexFormat.tangent || vertexFormat.bitangent) {
        ...
        var next = Cartesian2.Cartesian3.fromArray(
          positions,
          (i + 3) % length,
          scratchCartesian4
        );
        Cartesian2.Cartesian3.subtract(next, position, next);
        var bottom = Cartesian2.Cartesian3.subtract(
          extrudedPosition,
          position,
          scratchCartesian3
        );
        ...
        normal = Cartesian2.Cartesian3.normalize(
          Cartesian2.Cartesian3.cross(bottom, next, normal),
          normal
        );
  ...
  function computeExtrudedEllipse(options) {
    ...
    topBoundingSphere.center = Cartesian2.Cartesian3.add(
      center,
      scaledNormal,
    ...
    bottomBoundingSphere.center = Cartesian2.Cartesian3.add(
      center,
      scaledNormal,
    ...
    var topBottomIndices = IndexDatatype.IndexDatatype.createTypedArray(
      (posLength * 2) / 3,
      indices
    );
  ...
  function computeRectangle(
    ...
    if (rectangle.width > _Math.CesiumMath.PI) {
      rectangle.north =
        rectangle.north > 0.0
          ? _Math.CesiumMath.PI_OVER_TWO - _Math.CesiumMath.EPSILON7
          : rectangle.north;
      rectangle.south =
        rectangle.south < 0.0
          ? _Math.CesiumMath.EPSILON7 - _Math.CesiumMath.PI_OVER_TWO
          : rectangle.south;
...
//# sourceMappingURL=EllipseGeometry-44baa841.js.map
