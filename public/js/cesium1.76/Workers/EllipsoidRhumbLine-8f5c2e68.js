...
define(['exports', './when-ca071c93', './Check-802a3f11', './Math-439e03ae', './Cartesian2-be0369bc'], function (exports, when, Check, _Math, Cartesian2) { 'use strict';
  ...
  function calculateSigma(ellipticity, latitude) {
    ...
    return (
      Math.log(Math.tan(0.5 * (_Math.CesiumMath.PI_OVER_TWO + latitude))) -
  ...
  function calculateHeading(
    ...
    return Math.atan2(
      _Math.CesiumMath.negativePiToPi(secondLongitude - firstLongitude),
      sigma2 - sigma1
    );
  }
    ...
    if (
      ...
      if (major === minor) {
        distance =
          ...
          _Math.CesiumMath.negativePiToPi(deltaLongitude);
      } else {
  ...
  function computeProperties(ellipsoidRhumbLine, start, end, ellipsoid) {
    var firstCartesian = Cartesian2.Cartesian3.normalize(
      ellipsoid.cartographicToCartesian(start, scratchCart2),
      scratchCart1
    );
    var lastCartesian = Cartesian2.Cartesian3.normalize(
    ...
    Check.Check.typeOf.number.greaterThanOrEquals(
      "value",
      Math.abs(
        Math.abs(Cartesian2.Cartesian3.angleBetween(firstCartesian, lastCartesian)) - Math.PI
      ),
      0.0125
    );
    //>>includeEnd('debug');
    ...
    ellipsoidRhumbLine._start = Cartesian2.Cartographic.clone(
      start,
  ...
  function interpolateUsingSurfaceDistance(
    ...
    if (
      Math.abs(_Math.CesiumMath.PI_OVER_TWO - Math.abs(heading)) > _Math.CesiumMath.EPSILON8
    ) {
  ...
  EllipsoidRhumbLine.fromStartHeadingDistance = function (
    ...
    if (
      !when.defined(result) ||
      (when.defined(ellipsoid) && !ellipsoid.equals(result.ellipsoid))
    ) {
  ...
  EllipsoidRhumbLine.prototype.findIntersectionWithLongitude = function (
    ...
    if (Math.abs(_Math.CesiumMath.PI_OVER_TWO - absHeading) <= _Math.CesiumMath.EPSILON8) {
      ...
      _Math.CesiumMath.equalsEpsilon(
        Math.abs(_Math.CesiumMath.PI_OVER_TWO - absHeading),
        _Math.CesiumMath.PI_OVER_TWO,
        _Math.CesiumMath.EPSILON8
      )
    ) {
      ...
      result.latitude =
        _Math.CesiumMath.PI_OVER_TWO *
        _Math.CesiumMath.sign(_Math.CesiumMath.PI_OVER_TWO - heading);
      result.height = 0;
      return result;
    }
    ...
    var leftComponent =
      Math.tan(0.5 * (_Math.CesiumMath.PI_OVER_TWO + phi1)) *
...
//# sourceMappingURL=EllipsoidRhumbLine-8f5c2e68.js.map
