        ...
        bind: function () {
            ...
            $(document).on('click', (this._mouteclick = function (e) {
                ...
                if ((!$input && !$span && !$dropdown) ||
                    ($span && $span.get(0) !== that.$textspan.get(0)) ||
                    ($input && $input.get(0) !== that.$element.get(0)) ||
                    ($dropdown && $dropdown.get(0) !== that.$dropdown.get(0))) {
                    that.close(true);
                }
            ...
            $.each(data, function (i, n) {
                ...
                $.each(n, function (j, m) {
                    list.push(
                        ...
                        (m.selected ? ' active' : '') +
                        '">' +
                        ( simple ? that.simplize(m.address, PROVINCE) : m.address) +
                        '</a>');
                });
                list.push('</dd></dl>');
            });
        ...
        getList: function (data, type) {
            ...
            $.each(data, function (i, n) {
                list.push(
                    ...
                    (n.selected ? ' active' : '') +
                    '">' +
                    ( simple ? that.simplize(n.address, type) : n.address) +
                    '</a>');
            });
            list.push('</dd></dl>');
    ...
    $.fn.citypicker = function (option) {
        ...
        return this.each(function () {
            ...
            if (!data) {
                ...
                options = $.extend({}, that.data(), $.isPlainObject(option) && option);
                that.data(NAMESPACE, (data = new CityPicker(this, options)));
            }
